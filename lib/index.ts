...
export class CanvasTable {
  ...
  private static readonly TRANSPARENT_COLOR = !isNode
    ? "rgba(255,255,255,0)"
    : "transparent";
  ...
  private generateTitle(title: CTTitle): void {
    ...
    const lineHeight = Math.round(
      parseInt(title.fontSize, 10) * title.lineHeight
    );
    ...
    titleLines.forEach((line) => {
      ...
      innerLines.forEach((innerLine) =>
        ctx.fillText(innerLine, x + titleX, y + lineIndex++ * lineHeight)
      );
    });
  ...
  private generateRows(): void {
    ...
    for (let rowIndex = 0; rowIndex < data.length; rowIndex++) {
      const row = data[rowIndex];
      const lineHeight =
        cell.lineHeight * parseInt(cell.fontSize, 10) +
        cellPadding.bottom +
        cellPadding.top;
      this.x = tableStartX;
      for (const cellIndex in columnOuterWidths) {
        ...
        const option =
          header && !rowIndex ? header : { ...cell, ...columnOptions };
        ctx.font = `${option.fontWeight} ${option.fontSize} ${option.fontFamily}`;
        ctx.fillStyle = option.color;
        const textAlign =
  ...
  private generateFaders(): void {
    ...
    if (x > canvasWidth && fader.right) {
      var rightFader = ctx.createLinearGradient(
